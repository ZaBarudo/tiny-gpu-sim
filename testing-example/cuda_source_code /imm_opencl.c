...
int main(int argc, char** argv) {
    if (argc != 2) {
        fprintf(stderr, "Usage: %s <kernel_file.cl> <immediate_value>\n", argv[0]);
        return 1;
    }

    // Read immediate value from command line
    int imm = 10;
    ...
    // Copy the input data to memory buffer
    ...
    cl_kernel kernel = clCreateKernel(program, "add_imm", &ret);
    if (ret != CL_SUCCESS) {
        fprintf(stderr, "Failed to create kernel (error: %d). Make sure the kernel name is 'add_imm'\n", ret);
        exit(1);
    }

    // Set the arguments of the kernel
    ret = clSetKernelArg(kernel, 0, sizeof(cl_mem), (void *)&a_mem_obj);
    ret = clSetKernelArg(kernel, 1, sizeof(int), &imm);  // Correct way to pass scalar value
