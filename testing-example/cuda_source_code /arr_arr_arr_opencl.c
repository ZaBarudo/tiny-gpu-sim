...
int main(int argc, char** argv) {
    ...
    // Initialize data for the kernel
    ...
    cl_mem a_mem_obj = clCreateBuffer(context, CL_MEM_READ_ONLY, NUM_WORK_ITEMS * sizeof(int), NULL, &ret);
    cl_mem b_mem_obj = clCreateBuffer(context, CL_MEM_READ_ONLY, NUM_WORK_ITEMS * sizeof(int), NULL, &ret);
    cl_mem result_mem_obj = clCreateBuffer(context, CL_MEM_WRITE_ONLY, NUM_WORK_ITEMS * sizeof(int), NULL, &ret);

    // Copy the input data to memory buffers
    ret = clEnqueueWriteBuffer(command_queue, a_mem_obj, CL_TRUE, 0, NUM_WORK_ITEMS * sizeof(int), a, 0, NULL, NULL);
    ret = clEnqueueWriteBuffer(command_queue, b_mem_obj, CL_TRUE, 0, NUM_WORK_ITEMS * sizeof(int), b, 0, NULL, NULL);
    ...
    size_t global_item_size = NUM_WORK_ITEMS;  // 8 work-items total
    size_t local_item_size = NUM_WORK_ITEMS;   // 8 work-items in one group
    ret = clEnqueueNDRangeKernel(command_queue, kernel, 1, NULL, &global_item_size, &local_item_size, 0, NULL, NULL);

    // Read the memory buffer result from the device to the local variable result
    ret = clEnqueueReadBuffer(command_queue, result_mem_obj, CL_TRUE, 0, NUM_WORK_ITEMS * sizeof(int), result, 0, NULL, NULL);

    // Display all results (we only have 8)
    printf("Results from 8 work-items:\n");
    for (int i = 0; i < NUM_WORK_ITEMS; i++) {
        printf("index %d | a : %d | b : %d | result : %d\n", i, a[i], b[i], result[i]);
    }
